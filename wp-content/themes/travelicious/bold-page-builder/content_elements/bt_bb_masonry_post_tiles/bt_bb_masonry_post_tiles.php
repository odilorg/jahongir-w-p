...
class bt_bb_masonry_post_tiles extends BT_BB_Element {
	...
	function handle_shortcode( $atts, $content ) {
		...
		if ( $category_filter == 'yes' ) {
			...
			$output .= '<div class="bt_bb_post_grid_filter">';
				$output .= '<span class="bt_bb_post_grid_filter_item active" data-category-posts="" data-format-posts="' . esc_attr( $format ) . '">' . esc_html__( 'All', 'travelicious' ) . '</span>';
				foreach ( $cats as $cat ) {
					$output .= '<span class="bt_bb_post_grid_filter_item" data-category-posts="' . esc_attr( $cat->slug ) . '"  data-format-posts="' . esc_attr( $format ) . '">' . $cat->name . '</span>';
				}
			$output .= '</div>';
		}
	...
	function map_shortcode() {
		...
		bt_bb_map( $this->shortcode, array( 'name' => esc_html__( 'Masonry Post Tiles', 'travelicious' ), 'description' => esc_html__( 'Masonry tiles with post', 'travelicious' ), 'icon' => $this->prefix_backend . 'icon' . '_' . $this->shortcode,
			'params' => array(
				...
				array( 'param_name' => 'auto_loading', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'travelicious' ) => 'auto_loading' ), 'heading' => esc_html__( 'Load more items on scroll', 'travelicious' ), 'preview' => true
				),
				...
				array( 'param_name' => 'format', 'type' => 'textfield', 'preview' => true, 'heading' => esc_html__( 'Tiles format', 'travelicious' ), 'description' => esc_html__( 'e.g. 11, 21, 12, 22', 'travelicious' ), 'preview' => true
				),				
				array( 'param_name' => 'show_title', 'type' => 'checkbox', 'value' => array( esc_html__( 'Yes', 'travelicious' ) => 'show_title' ), 'heading' => esc_html__( 'Show title', 'travelicious' ), 'preview' => true
				),
